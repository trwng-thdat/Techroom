...
Keep shared demo data in each site root `data.js` file instead of duplicating arrays in page scripts. For teacher pages, read from `window.TeacherData`; for admin/owner pages, read from `window.OwnerData`; for student pages, use the same pattern with `student-site/data.js` when data is needed.
...
Then visit `http://localhost:8000/student-site/`, `http://localhost:8000/teacher-site/`, or `http://localhost:8000/owner-site/`.
...
For teacher and owner pages, also confirm the shared sidebar/header still render and the correct active state or back link is preserved.
