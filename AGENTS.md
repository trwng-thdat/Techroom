...
Keep shared demo data in each site root `data.js` file instead of duplicating arrays in page scripts. For teacher pages, read from `window.TeacherData`; for student pages, use the same pattern with `student-site/data.js` when data is needed.
...
For teacher pages, also confirm the shared sidebar/header still render and the correct active state or back link is preserved.
